...
class Game:
	def __init__(
		...
		self._screen = screen or pygame.display.set_mode(self.size, pygame.RESIZABLE)
	...
	def update(self) -> None:
		...
		if self._player.is_game_over():
			...
			if self._player.should_spawn_debris() and not self._debris_spawned:
				...
				sorted_debris = sorted(debris_list, key=lambda obj: obj.depth)
				self._debris_particles = ObjectOrderedSet(*sorted_debris)
